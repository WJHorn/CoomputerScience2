...
public:
   ...
   void Optimize()
   {
        int left = rootPtr->data;
        int right = getDepth();
        OptimizeHelper(rootPtr, left, right);
   }
private:
   ...
   void preOrderHelper(TreeNode<NODETYPE>* ptr) const {
		 //left shift 8 bits       if (ptr != nullptr) {
   ...
   //the optimizeHelper will return the new binary tree
   TreeNode<NODETYPE>* OptimizeHelper(TreeNode<NODETYPE>* nodePtr, int first, int last)
   {
       if (first > last)
       {
           return nullptr;
       ...
       OptimizeHelper(nodePtr->leftPtr, first, mid - 1);
       OptimizeHelper(nodePtr->rightPtr, mid + 1, last);
